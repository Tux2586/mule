...
public class MessageEnricherTestCase extends AbstractMuleContextTestCase {
  ...
  public void enricherConservesSameEventInstance() throws Exception {
    ...
    MuleEvent in =
        new DefaultMuleEvent(new DefaultMessageExecutionContext(muleContext.getUniqueIdString(), null),
                             MuleMessage.builder().payload(TEST_MESSAGE).build(), MessageExchangePattern.REQUEST_RESPONSE, flow);
    MuleEvent out = enricher.process(in);
  ...
  private MuleEvent createNonBlockingEvent(SensingNullReplyToHandler nullReplyToHandler) {
    ...
    return new DefaultMuleEvent(new DefaultMessageExecutionContext(muleContext.getUniqueIdString(), null),
                                MuleMessage.builder().payload(TEST_MESSAGE).build(), MessageExchangePattern.REQUEST_RESPONSE,
