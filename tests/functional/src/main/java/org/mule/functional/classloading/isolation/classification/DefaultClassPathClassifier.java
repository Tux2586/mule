...
public class DefaultClassPathClassifier implements ClassPathClassifier {
   ...
   * for each artifact that contains the {@value #SERVICE_PROPERTIES_FILE_NAME} file. Once an artifact is identified the
   * dependencies are going to be collected from the graph using Maven {@code provided} scope.
   *
   * @param extendedContext {@link ExtendedClassPathClassifierContext} that holds the data needed for classifying the artifacts
  ...
  private List<ServiceUrlClassification> buildServicesUrlClassification(final ExtendedClassPathClassifierContext extendedContext) {
    ...
    try {
      ...
      for (Resource resource : resources) {
        ...
        } else if (protocol.equals(FILE) && resource.getFile().isDirectory()) {
        ...
        logger.debug("Service: '" + serviceProviderClassName + "' found and being classified");
  ...
  /**
   * For the given artifact {@link File} it will lookup for the maven artifact id using the {@link MavenMultiModuleArtifactMapping}.
   *
